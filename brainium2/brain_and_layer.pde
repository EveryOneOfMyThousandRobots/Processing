class Layer {
  ...
  Layer(int prev, int howMany) {
    weights = new float[prev][howMany];
...
class Brain {
  ...
  void calc() {
    for (int i = 0; i < l1.values.length; i += 1) {
      for (int j = 0; j < l1.weights.length; j += 1) {
        l1.values[i] = l1.weights[j][i] * inputs[j];
        l1.values[i] = 1 / (1 + pow(exp(1), -l1.values[i]));
      }
    }
    for (int i = 0; i < l2.values.length; i += 1) {
      for (int j = 0; j < l2.weights.length; j += 1) {
        l2.values[i] = l2.weights[j][i] * l1.values[j];
        l2.values[i] = 1 / (1 + pow(exp(1), -l2.values[i]));
      }
    }
    for (int i = 0; i < out.values.length; i += 1) {
      for (int j = 0; j < out.weights.length; j += 1) {
        out.values[i] = out.weights[j][i] * l2.values[j];
